 ...
 *YES
 */
@Autonomous(name= "autoPh_red_skystone", group="AutoOP")
//@Disabled//comment out this line before using
public class autoPh_red_skystone extends robot {
...
//            moveDistance(0.4, 700);
        ...
        waitForStart();{
            ...
            telemetry.update();
            telemetry.addData("Values", valLeft+"   "+valMid+"   "+valRight);
            telemetry.addData("Values l,m,r", l+"   "+m+"   "+r);
            telemetry.addData("Skystone Position: ", pologenieskystone);
            telemetry.update();
           ...
           while(true) {
                if (l==valLeft&&m==valMid&&r==valRight){
                    telemetry.addData("We found second skystone ",' ');
                    telemetry.update();
                }
            }*/
            ...
            {
                setMotorsPowerTimed(0.2*koeff,-0.2*koeff,-0.23*koeff,0.24*koeff,1570);//vpered
                sleep(200);

                setMotorsPowerTimed(-0.2*koeff,0.2*koeff,-0.2*koeff,0.2*koeff,600);//vlevo
                sleep(200);
            ...
            if(pologenieskystone=="right")
            {
                ...
                setMotorsPowerTimed(0.2*koeff,-0.2*koeff,-0.23*koeff,0.24*koeff,1570);//vpered
                sleep(200);
                setMotorsPowerTimed(0.2*koeff,-0.2*koeff,0.2*koeff,-0.2*koeff,670);//vpravo
                sleep(200);

                goforskystoneopencv(koeff);
            }
            setMotorsPowerTimed(-0.23*koeff,0.23*koeff,0.23*koeff,-0.23*koeff,1800);//nazad
            sleep(200);
            setMotorsPowerTimed(0.3*koeff,-0.3*koeff,0.3*koeff,-0.3*koeff,600*pologenieskystone1+1300);//vpravo
            sleep(200);
            otpustiskystone();
            setMotorsPowerTimed(-0.23*koeff,0.25*koeff,-0.2*koeff,0.2*koeff,1000);//vlevo

            int w2=1;
            while(w2==1){
                setMotorsPower(-0.23,0.25,-0.2,0.2);
                if(l==valLeft&&m==valMid&&r==valRight){
                    w2=0;
            ...
            setMotorsPowerTimed(0,0,-0.35,0.35,1400);
            setMotorsPowerTimed(-0.25*koeff,0.27*koeff,-0.2*koeff,0.2*koeff,700);//vlevo
            sleep(200);
            if(pologenieskystone=="left")
            {
                setMotorsPowerTimed(0.2*koeff,-0.2*koeff,0.2*koeff,-0.2*koeff,250);//vpravo
                sleep(200);
                goforskystoneopencv(koeff);
            }
            if(pologenieskystone=="middle")
            ...
            if(pologenieskystone=="right")
            {
                setMotorsPowerTimed(0.2*koeff,-0.2*koeff,0.2*koeff,-0.2*koeff,850);//vpravo
                sleep(200);
                goforskystoneopencv(koeff);
            }
            setMotorsPowerTimed(0,0,0.35,-0.35,1500);
            sleep(100);
            setMotorsPowerTimed(-0.2*koeff,0.2*koeff,0.2*koeff,-0.2*koeff,500);//nazad
            sleep(100);
            setMotorsPowerTimed(0.3*koeff,-0.3*koeff,0.3*koeff,-0.3*koeff,800+1500);//vpravo
            sleep(200);
            otpustiskystone();
            setMotorsPowerTimed(-0.3*koeff,0.3*koeff,-0.3*koeff,0.3*koeff,1300);//vlevo
            sleep(200);
        }
    }
    //------------------------------------------------------------------------------------------------------------------
